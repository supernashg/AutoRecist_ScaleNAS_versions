...
def parse_args():
    ...
    parser.add_argument('--DP',
                        action='store_true',
                        help='DataParallel instead of DistributedDataParallel')
...
def main():
    ...
    gpus = list(cfg.GPUS)
    distributed = len(gpus) > 1 and ( not hasattr(args, 'DP') )
    ...
    batch_size_mulitplier = len(gpus) if hasattr(args , 'DP') else 1

    trainloader = torch.utils.data.DataLoader(
        train_dataset,
        batch_size=cfg.TRAIN.BATCH_SIZE_PER_GPU*batch_size_mulitplier,
    ...
    valid_loader = torch.utils.data.DataLoader(
        test_dataset,
        batch_size=cfg.TEST.BATCH_SIZE_PER_GPU*batch_size_mulitplier,
        shuffle=False,
        num_workers=cfg.WORKERS,
        pin_memory=True,
    ...
    elif hasattr(args, 'DP'):
        print('TODO: DP of SyncBatchNorm')
    else:
        print('No parallel Sync for BatchNorm')
    ...
    if distributed:
        ...
        model = nn.DataParallel(model, device_ids=list(cfg.GPUS)).cuda()
    else:
        print('No model parallelism')
