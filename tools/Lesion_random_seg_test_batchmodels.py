...
from core.seg_function import validate_seg_batchmodels
...
def parse_args():
    ...
    parser.add_argument('--depth_list',
                        type=str,
                        default=[2, 3, 4, 5])
    parser.add_argument('--batchmodels',
                        type=int,
                        default=3)
...
def main():
    ...
    model_list = []
    for j in range(args.batchmodels):
        if j == 0:
    ...
    for i in range( int( np.ceil(args.sample_num/args.batchmodels) )) :
        results = []
        for j in range(args.batchmodels):
            ...
            random.seed(seed)
            model_list[j].module.set_fusion_percentage(fusion_percentage)
            model_list[j].module.set_depth_list(depth_list)
            subnet_setting = model_list[j].module.sample_active_subnet()
            ...
            if args.bn_calib:
                calib_bn(cfg, model_list[j], seed)
            params, flops, details = get_model_summary(model_list[j].module.model, dump_input.cuda())
        ...
        mIoUs= validate_seg_batchmodels(cfg, testloader, model_list, writer_dict, device=None)
