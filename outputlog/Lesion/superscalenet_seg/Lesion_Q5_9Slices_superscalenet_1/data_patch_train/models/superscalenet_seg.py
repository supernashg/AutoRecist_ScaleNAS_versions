...
class SuperScaleNet_Seg(nn.Module):
    ...
    def __init__(self, config, **kwargs):
        ...
        # stem net
        self.conv1 = nn.Conv2d(cfg.LESION.SLICE_NUM, 64, kernel_size=3, stride=2, padding=1,
                               bias=False)#change to number of input slices
    ...
    def init_weights(self, pretrained='', ):
        ...
        if os.path.isfile(pretrained):
            ...
            for different_shape_k in ['conv1.weight']:
                if model_dict[different_shape_k].size() != pretrained_dict[different_shape_k].size() :
                    #torch.Size([64, 3, 3, 3]) != torch.Size([64, 9, 3, 3])
                    if model_dict[different_shape_k].size(1) % pretrained_dict[different_shape_k].size(1) :
                        print('ERROR model size of {} should be exactly divisible!'.format(different_shape_k))
                        break
                    ratio = model_dict[different_shape_k].size(1) // pretrained_dict[different_shape_k].size(1) 
                    pretrained_dict[different_shape_k] = pretrained_dict[different_shape_k].repeat(1,ratio,1,1)
